...
import React3, { useEffect as useEffect2, useMemo, useState as useState2 } from "react";
...
var TokenKitProvider = ({ children, mainnetNodeURL, sepoliaNodeURL, network }) => {
  ...
  const [connection, setConnection] = useState2();
  const [account, setAccount] = useState2();
  const [address, setAddress] = useState2("");
  const [loading, setLoading] = useState2(false);
  const [network_, setNetwork] = useState2(network);
  ...
  return /* @__PURE__ */ React3.createElement(TokenKitContext.Provider, { value: contextValue }, children);
};
...
export {
  ...
  Modal_default as SelectTokenModal,
